...
impl Vector {
    fn reaches(&self, target: &Point) -> bool {
        for amount in 1..self.magnitude {
            if self.apply(amount) == *target {
                return true
            }
        }
        return false
    ...
    fn apply(&self, amount: i32) -> Point {
        match self.dir {
            Direction::Up => Point{
                x: self.origin.x,
    ...
    fn intersect(&self, other: &Vector) -> Option<Point> {
        let (a, b) = self.origin.collisions_from(&other.origin);

        if self.reaches(&a) && other.reaches(&a) {
            Some(a)
        } else if self.reaches(&b) && other.reaches(&b) {
            Some(b)
        } else {
            None
        }
...
#[test]
fn test_vector_intersect_orthogonal() {
    let vec1 = Vector{
        dir: Direction::Up,
        magnitude: 3,
    ...
    let inter = vec1.intersect(&vec2);
    assert!(inter.is_none());
}

#[test]
fn test_vector_intersect_intersect() {
    let vec1 = Vector{
        dir: Direction::Right,
        magnitude: 3,
...
impl Point {
    ...
    fn collisions_from(&self, other: &Point) -> (Point, Point) {
        (Point{ x: self.x, y: other.y }, Point{ x: other.x, y: self.y })
    }
    ...
    fn to_vec(&self, vec: &str) -> Result<Vector, String> {
        let next = vec.chars().next().unwrap();
        let num = vec.chars().skip(1).collect::<String>().parse::<i32>().unwrap();
        ...
        match next {
            ...
            _ => {
                return Err("unexpected direction".to_string());
            }
        }
...
fn vectors(path: &str) -> Vec<Vector> {
    let segments = path.split(",");
    let mut vecs: Vec<Vector> = Vec::new();
    let mut cur = Point::origin();

    for segment in segments {
        let next = cur.to_vec(segment).unwrap();
        cur = next.apply(next.magnitude);
        vecs.push(next);
    }
...
fn min_dist(vec: Vec<Point>) -> i32 {
    let cols = vec.iter().map(|&c| c.dist(&Point::origin())).collect::<Vec<i32>>();
    return min(cols);
}

fn min(vec: Vec<i32>) -> i32 {
    let mut min: i32 = *vec.first().unwrap();
...
fn test_min_dist_1() {
    ...
    assert_eq!(collide(path1, path2), 135);
}
...
fn test_min_dist_2() {
    ...
    assert_eq!(collide(path1, path2), 6);
}
...
fn test_min_dist_3() {
    ...
    assert_eq!(collide(path1, path2), 159);
}

fn collide(path1: &str, path2: &str) -> i32 {
    let vecs1 = vectors(path1);
    let vecs2 = vectors(path2);

    let cols = collisions(&vecs1, &vecs2);
    return min_dist(cols);
}

fn collisions(vecs1: &[Vector], vecs2: &[Vector]) -> Vec<Point> {
    let mut collisions: Vec<Point> = Vec::new();

    for vec1 in vecs1 {
        for vec2 in vecs2 {
            if let Some(collision) = vec1.intersect(&vec2) {
...
fn steps(path: &[Vector], target: &Point) -> i32 {
    let mut steps = 0;

    for vec in path {
        if vec.reaches(target) {
            let dist = vec.origin.dist(target).abs();
            steps += dist;
            break;
        } else {
            steps += vec.magnitude;
...
#[test]
fn test_min_step_cost() {
    let path1 = vectors("R8,U5,L5,D3");
    let path2 = vectors("U7,R6,D4,L4");

    assert_eq!(min_step_cost(&path1, &path2), 30);
}

#[test]
fn test_min_step_cost_2() {
    let path1 = vectors("R75,D30,R83,U83,L12,D49,R71,U7,L72");
    let path2 = vectors("U62,R66,U55,R34,D71,R55,D58,R83");

    assert_eq!(min_step_cost(&path1, &path2), 610);
}

fn min_step_cost(vecs1: &[Vector], vecs2: &[Vector]) -> i32 {
    let cols = collisions(&vecs1, &vecs2);

    let mut stepvec: Vec<i32> = Vec::new();

    for col in cols {
        let steps1 = steps(&vecs1, &col);
        let steps2 = steps(&vecs2, &col);

        stepvec.push(steps1 + steps2);
    }
